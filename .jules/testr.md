...
2025-05-27 - [Adversarial Constraints via Anti-Correlation]
Learning: Algorithms modeling physical processes often have strict constraints (e.g., crosstalk is strictly additive, so unmixing coefficients cannot be positive). Purely mathematical optimization (like blind source separation minimizing Mutual Information) might find mathematically optimal but physically impossible solutions if unconstrained. Testing these boundaries with strongly adversarial inputs—like strictly anti-correlated signals that *should* mathematically unmix with negative crosstalk—is a powerful way to verify that physical constraints override the mathematical optimizer.
Action: Implement tests that feed adversarial, unphysical relationships (like negative correlation in an additive-only system) to verify the algorithm correctly clamps bounds and refuses to hallucinate impossible parameters.
