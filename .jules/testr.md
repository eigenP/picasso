...
2025-05-27 - [Physical Positivity Constraints and Anti-Correlation]
Learning: Blind source separation on physical signals must enforce physical bounds. Fluorescence crosstalk strictly adds photons; it never subtracts them. Thus, the unmixing matrix off-diagonals must be strictly non-positive. Unconstrained optimization might attempt to hallucinate "negative crosstalk" for anti-correlated signals to minimize Mutual Information. Verifying that the algorithm safely clamps these unphysical updates ensures it operates within reality.
Action: Implement bounding and constraint tests using adversarial (anti-correlated) inputs to verify that the algorithm strictly refuses to apply unphysical optimizations.
