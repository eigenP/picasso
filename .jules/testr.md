...
2025-05-27 - [Physical Bounds Constraint & Adversarial Testing]
Learning: Optimization algorithms modeling real-world physics (like additive fluorescence crosstalk) must sometimes intentionally reject pure mathematical optimums if they violate physical reality (like "negative crosstalk"). When testing these constraints, it is insufficient to use realistic data; instead, you must explicitly construct adversarial inputs (like heavily anti-correlated signals) designed specifically to tempt the algorithm into violating the physical bounds. This proves the constraint logic actively overrides the optimization engine when necessary.
Action: When verifying physical constraints, intentionally generate adversarial inputs that perfectly fit the "unphysical" mathematical solution, and assert that the algorithm correctly identifies and safely clamps the result to a physically valid fallback state.
