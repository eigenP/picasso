...
2025-05-27 - [Domain Constraints vs Mathematical Optimization]
Learning: Mathematical optimization (like minimizing Mutual Information) will blindly exploit any structure in the data, including unphysical correlations (like negative crosstalk/anti-correlation). In systems modeling physical processes (e.g., additive fluorescence), the algorithm must enforce physical bounds (e.g., $U_{ij} \le 0$) to prevent hallucinating unphysical states. A pure mathematical test won't catch this failure mode.
Action: To test that domain-specific physical constraints are strictly enforced and override raw optimization, explicitly construct adversarial, unphysical inputs (like strongly anti-correlated signals) and verify that the algorithm successfully rejects them by clamping coefficients.
