...
2025-05-27 - [Physical Bounds Override Mathematical Optima]
Learning: When algorithms optimize a mathematical objective (like Mutual Information) to model a physical process, the objective function itself is often blind to physical reality. For example, fluorescence crosstalk is strictly additive; negative crosstalk is unphysical. If fed adversarial or anomalous data (like perfectly anti-correlated signals), a pure mathematical optimizer will happily hallucinate unphysical coefficients (e.g., negative mixing / positive unmixing) to minimize the metric. The system's correctness relies entirely on explicit physical bounds (clamping) to override the math. Verifying these bounds requires deliberately passing adversarial, unphysical inputs and ensuring the algorithm safely rejects them (e.g., returning Identity) rather than succeeding with an unphysical result.
Action: Always write specific tests that pit the mathematical optimizer against the physical constraints using adversarial data, confirming the constraints win.
