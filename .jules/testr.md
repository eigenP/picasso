...
2025-05-27 - [Physical Constraints and Adversarial Rejection]
Learning: Pure mathematical optimization can hallucinate unphysical solutions when given adversarial or out-of-distribution data (like anti-correlated signals in an additive-only system). Enforcing and explicitly testing hard physical boundaries (like $U_{ij} \le 0$) is critical to ensure the algorithm safely degrades to a null-op (Identity matrix) rather than returning mathematically valid but physically impossible results.
Action: When algorithms model physical processes with directional constraints (e.g., only additive crosstalk), test them with strictly adversarial inputs (anti-correlated data) to confirm that the bounding logic correctly overrides the optimization objective and prevents unphysical hallucinations.
