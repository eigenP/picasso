...
2025-05-27 - [Physical Bounds vs Mathematical Optimization]
Learning: When algorithms model strictly additive physical processes (like fluorescence crosstalk), pure mathematical optimization (e.g., minimizing Mutual Information) might attempt to violate physical bounds to achieve a "better" mathematical score. For example, given explicitly anti-correlated inputs, the optimizer might attempt to hallucinate unphysical negative crosstalk (positive unmixing coefficients) to destroy the anti-correlation. Explicitly testing with adversarial, unphysical inputs is necessary to ensure the algorithm prioritizes physical constraints (like strictly non-positive off-diagonals) over mathematical optimization.
Action: When verifying algorithms modeling physical processes, explicitly test with adversarial or unphysical inputs to ensure the algorithm safely rejects them and correctly clamps bounds, overriding pure mathematical optimization.
