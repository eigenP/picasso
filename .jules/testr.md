...
2025-05-26 - [Algorithmic Monotonicity]
Learning: Coordinate descent algorithms must monotonically decrease their objective function at every single iteration. For unmixing, checking that Total Correlation (sum of pairwise Mutual Information) never increases provides strict verification that the pairwise `minimize_mi` step correctly reduces the global objective. If this property is violated, either the local update is flawed or the local steps are interfering detrimentally with the global objective.
Action: For iterative optimization algorithms, implement tests that track the global objective function at each iteration and strictly assert monotonic descent (within numerical tolerances), rather than only checking the final converged output.
