...
2025-05-27 - [Physical Constraints with Adversarial Inputs]
Learning: When verifying physical constraints in algorithms modeling physical processes (like additive fluorescence crosstalk), it's not enough to test with valid data. You must explicitly test with adversarial or unphysical inputs (like strictly anti-correlated signals). A pure mathematical optimization (minimizing Mutual Information) might attempt to hallucinate unphysical negative crosstalk (positive unmixing coefficients) to further reduce the objective. The algorithm must recognize this, correctly clamp the bounds, and override the pure mathematical optimization to safely reject the hallucinated structure (returning an Identity matrix).
Action: For physics-constrained algorithms, design adversarial tests that push the inputs into unphysical regimes to ensure the algorithm safely clamps bounds and refuses to produce mathematically optimal but physically invalid results.
