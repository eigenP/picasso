...
2025-05-27 - [Adversarial Testing for Physical Constraints]
Learning: When algorithms modeling physical processes (like fluorescence spectral unmixing) use purely mathematical optimization (like minimizing Mutual Information), they can "hallucinate" unphysical solutions if the inputs are adversarial. For example, given strictly anti-correlated signals (which naturally occur in mutually exclusive biological compartments), an unbounded optimizer will attempt to introduce "negative crosstalk" (positive off-diagonal matrix elements) to destroy the anti-correlation. The algorithm enforces a strict physical positivity constraint ($U_{ij} \\le 0$) to prevent this, because fluorescence crosstalk is strictly additive. Testing this boundary condition explicitly with adversarial, hand-crafted anti-correlated inputs ensures the constraint correctly overrides the raw optimization logic.
Action: When verifying physical constraints in algorithms, explicitly test with adversarial or unphysical inputs to ensure the algorithm safely rejects them and correctly clamps bounds, overriding pure mathematical optimization.
