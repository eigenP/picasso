...
Learning: Algorithms modeling physical systems (like fluorescence unmixing) must rigorously enforce physical boundaries (like non-negativity of additive signals) against the purely mathematical optimization. Supplying heavily anti-correlated adversarial inputs exposes whether the algorithm might "hallucinate" unphysical negative crosstalk just to minimize statistical dependence. Testing these adversarial limits proves that the mathematical optimization is correctly subordinated to physical reality.
Action: Implement tests that feed adversarial data designed to break the physical assumptions of the generative model, and assert that the algorithm safely refuses them and correctly clamps to safe bounds (e.g., returning the Identity matrix).
