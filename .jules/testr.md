...
2025-05-27 - [Verifying Physical Constraints with Adversarial Inputs]
Learning: When algorithms optimize a mathematical objective (like Mutual Information) to model a physical process (like fluorescence), they must correctly enforce the physical constraints of that process. Fluorescence crosstalk is strictly additive; you cannot have "negative photons." Therefore, the unmixing matrix must only subtract signal (off-diagonals <= 0). Pure mathematical optimization, if unbounded, would happily add anti-correlated channels together to falsely reduce entropy. We learned that the algorithm correctly clamps these attempts.
Action: When verifying physical constraints in algorithms modeling physical processes, explicitly test with adversarial or unphysical inputs (like strictly anti-correlated signals in an additive-only system) to ensure the algorithm safely rejects them and correctly clamps bounds, overriding pure mathematical optimization.
