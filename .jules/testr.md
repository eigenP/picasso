...
2025-05-28 - [Spatial Invariance / Bag-of-Pixels]
Learning: The unmixing algorithm optimizes the joint distribution (Mutual Information) via global histograms, making it fundamentally agnostic to spatial organization. Verifying this "bag of pixels" invariant ensures that the algorithm handles arbitrary dimensional shapes (1D time-series, 2D images, 3D volumes) identically and is immune to permutation of pixels.
Action: Implement spatial invariant tests to guarantee independence from ordering and dimensionality, ensuring consistent behavior across diverse data representations without needing shape-specific checks.
