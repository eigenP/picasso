...
2025-05-27 - [Physical Constraints as Adversarial Bounds]
Learning: Unconstrained optimization algorithms can hallucinate "unphysical" solutions when presented with adversarial or edge-case inputs (e.g., trying to "unmix" anti-correlated signals by adding them together). When an algorithm models a physical process (like spectral crosstalk, which is strictly additive), its correctness relies heavily on enforcing physical bounds (e.g., non-positive unmixing coefficients). Testing these bounds against artificially adversarial inputs ensures the algorithm acts sensibly and fails safely in extreme regimes.
Action: For optimization algorithms with physical constraints, design tests that supply adversarial inputs specifically crafted to push the unconstrained objective toward unphysical solutions, and assert that the bounds strictly clamp the output to physically plausible values.
