...
2025-05-27 - [Physical Bounds vs Pure Optimization]
Learning: When an algorithm models a strictly additive physical process (like fluorescence crosstalk), pure mathematical optimization (like minimizing Mutual Information) can be tricked by adversarial inputs (like anti-correlated signals) into suggesting unphysical solutions (e.g., negative crosstalk). Testing that the algorithm strictly enforces its physical bounds (clamping unphysical updates) is crucial, as it distinguishes a robust scientific tool from a naive optimizer.
Action: For algorithms with physical constraints, always implement tests using adversarial or unphysical inputs that mathematically favor violating those constraints, and assert that the bounds are successfully enforced, overriding the pure optimization objective.
