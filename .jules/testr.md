...
2025-05-27 - [Positivity Constraint / Additive Crosstalk]
Learning: Fluorescence crosstalk is strictly additive; light leakage adds signal, it never subtracts it. Therefore, an unmixing matrix must have strictly non-positive off-diagonal elements ($U_{ij} \le 0$). When presented with anti-correlated signals, a naive statistical decorrelation would attempt to add them (resulting in $U_{ij} > 0$) to minimize mutual information. The algorithm must recognize this physical impossibility and clamp such updates, refusing to hallucinate "negative crosstalk".
Action: Implement tests with adversarial, anti-correlated inputs to verify that the algorithm enforces the physical positivity constraint and correctly clamps updates (yielding an Identity matrix) rather than blindly optimizing the statistical objective.
