...
2025-05-27 - [Verifying Physical Bounds over Mathematical Optima]
Learning: Scientific algorithms modeling physical reality (like additive spectral crosstalk) often impose constraints on optimization (like $U_{ij} \le 0$). Pure mathematical test cases (e.g., standard correlated data) rarely hit these bounds. To verify that bounds-checking logic is actually working and preventing the hallucination of "unphysical" solutions, one must feed the algorithm adversarial or "anti-physical" inputs (e.g., negative mixing matrices or perfectly anti-correlated signals) where the mathematical optimum violates the physics.
Action: When testing algorithms with physical constraints, implement "anti-physical" adversarial tests that force the mathematical optimization into forbidden territory, strictly asserting that the algorithm clamps the result to safe bounds (e.g., returning an Identity matrix).
