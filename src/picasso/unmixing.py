...
def minimize_mi(
    ...
    *,
    ...
    # The lower bound is strictly 0.0 (fluorescence crosstalk is additive).
    # The upper bound is mathematically constrained by the theoretical mixing
    # matrix if provided, otherwise bounded loosely by 1.0.
    ub = upper_bound if upper_bound is not None else 1.0

    # Ensure bounds are strictly ordered (in case of a 0.0 upper bound)
    if ub < 0.0:
        ub = 0.0
    ...
    # Calculate baseline MI before any updates
    base_mi = func(0.0)
    ...
    # Statistical thresholding: Do not accept unmixing updates that only fit
    # the noise floor of the empirical histogram Mutual Information estimator.
    # The expected asymptotic bias/variance of empirical entropy (Miller-Madow)
    # is bounded roughly by bins / (2 * N). If the MI reduction is smaller than
    # this noise floor, the step is statistically indistinguishable from 0.
    # Rejecting it prevents hallucinated correlations and over-unmixing.
    n_samples = x.size
    noise_floor = bins / (2.0 * n_samples)

    if base_mi - result.fun < noise_floor:
        return 0.0
