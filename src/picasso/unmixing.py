...
def minimize_mi(
    ...
    *,
    ...
    ub = 1.0 if upper_bound is None else upper_bound
    if ub <= 0.0:
        return 0.0

    # optimize using Brent's method. Note that method='bounded' does not use `bracket`
    # and therefore init_alpha cannot be passed.
    res = minimize_scalar(
        func,
        bounds=(0.0, ub),
        method='bounded',
    ...
    # Compute Miller-Madow bias to prevent algorithm from hallucinating unphysical unmixing
    # coefficients due to entropy binning artifacts.
    N = x.size
    bias_threshold = bins / (2 * N)

    mi_initial = func(0.0)
    mi_opt = res.fun

    if (mi_initial - mi_opt) <= bias_threshold:
        return 0.0
